...
import zkshieldPolygonMumbai from "../contracts/zkshieldPolygonMumbai.json"
...
const RequestsScreen = ({ drawerWidth }) => {
    ...
    const sendRequest = async () => {
        ...
        const providerContract = new ethers.Contract(
            zkshieldPolygonMumbai.address,
            zkshieldPolygonMumbai.abi,
            provider
        )
    ...
    const downloadProof = async (cid) => {
        ...
        try {
            console.log("This is the cid of the proof - " + cid)
            const resp = await axios({
                method: "get",
            ...
            setSnackbarSeverity("error")
            setSnackbarMessage(
                "Oops in downlaoding something went wrong in downloading proof! Please try again"
            )
        }
        setIsSnackbarOpen(true)
        for (let i = 0; i < newOutgoingData.length; i++) {
