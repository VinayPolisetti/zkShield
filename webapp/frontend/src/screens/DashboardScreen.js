...
const DashboardScreen = ({ drawerWidth }) => {
        ...
        const innerFunction = async () => {
            try {
                ...
                if (!("error" in walletResp.data)) {
                    ...
                    setSnackbarMessage(
                        "HereOops something went wrong in fetching data! Please reload"
                    )
                    setIsSnackbarOpen(true)
                }
            } catch (error) {
    ...
    useEffect(() => {
        if (walletsData != null) {
            const innerFunction = async () => {
                try {
                    const priceResp = await axios({ //To get current market price details for Mumbai
                        method: "get",
                        ...
                        params: {
                            tokens: priceTokens.tokens, //Not a jwt token but token addresses
                    ...
                    if (!("error" in priceResp.data)) {
                        ...
                        setSnackbarSeverity("error")
                        setSnackbarMessage(
                            "MyOops something went wrong in fetching data! Please reload"
                        )
                        setIsSnackbarOpen(true)
                    }
                } catch (error) {
    ...
    useEffect(() => {
        if (priceData != null) {
            setApiCounter(walletsData.length * quantityTokens.length) // For each wallet, check both mumbai and sepolia
        }
    }, [priceData])
                ...
                const innerFunction = async () => {
                    ...
                    try {
                        ...
                        if (!("error" in quantityResp.data)) {
                            ...
                            console.log(quantityResp);
                            setSnackbarSeverity("error")
                            setSnackbarMessage(
                                "3rd1Oops something went wrong in fetching data! Please reload"
                            )
                            setIsSnackbarOpen(true)
                        }
                    } catch (error) {
                        setSnackbarSeverity("error")
                        setSnackbarMessage(
                            "3rd2Oops something went wrong in fetching data! Please reload"
                        )
                        setIsSnackbarOpen(true)
                    }
                }
